...
| `/schedule` | 📅 schedule | Weekly timetable grouped by day, with room and professor |
| `/attendance` | ✅ attendance | Per-course attendance breakdown with percentage and color indicators |
| `/grades` | 🎓 grades | Grades and GPA per course per semester |
| `/gpa` | 📊 GPA | Cumulative GPA, per-semester breakdown, and grade distribution |
| `/courses` | 📚 courses | Registered courses with session booking status |
| `/enroll` | 📋 enroll | Enroll in an available session (Lecture / Section / Lab) |
| `/regcourse` | ➕ regcourse | Register for a new course in the student's department |
| `/feedback` | ⭐ feedback | Multi-step professor rating (1–5 stars + optional comment) |
| `/profile` | 👤 profile | Full student profile (name, ID, department, faculty, year) |
| `/menu` | — | Show main inline keyboard |
| `/help` | ℹ️ help | Display all available commands |
